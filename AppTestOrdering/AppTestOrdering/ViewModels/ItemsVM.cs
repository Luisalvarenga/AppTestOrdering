...
namespace AppTestOrdering.ViewModels
{
    public class ItemsVM : INotifyPropertyChanged
    {
		public static ItemsVM Current { get; } = new ItemsVM();

		public ICommand FinishProcessCommand { get; set; }
        public ICommand OpenListMonkeysCommand { get; set; }
        public ICommand SelectionMonkeyChangedCommand { get; set; }

        private ObservableCollection<Monkey> _ListMonkeys;
        private ObservableCollection<object> _SelectedListMonkeys;


        public ObservableCollection<Monkey> ListMonkeys { get { return _ListMonkeys; } set { _ListMonkeys = value; } }
        public ObservableCollection<object> SelectedListMonkeys { get { return _SelectedListMonkeys; } set { _SelectedListMonkeys = value; } }

        public ItemsVM()
        {
			FinishProcessCommand = new Command(ViewSelectedMonkeys);
			OpenListMonkeysCommand = new Command(OpenListMonkeys);
			SelectionMonkeyChangedCommand = new Command(MonkeySelected);

			CreateMonkeyCollection();
		}

        private void MonkeySelected(object obj)
        {
			if (SelectedListMonkeys == null)
				return;

			Debug.Print($"Monkeys selected now: {SelectedListMonkeys.Count()}");
        }

        private void OpenListMonkeys(object obj)
        {
			App.Current.MainPage.Navigation.PushAsync(new Views.SelectItems());
        }

        void CreateMonkeyCollection()
		{
			ListMonkeys = new ObservableCollection<Monkey>();
			SelectedListMonkeys = new ObservableCollection<object>();

			ListMonkeys.Add(new Monkey
			{
				Name = "Baboon",
		...
		private void ViewSelectedMonkeys(object obj)
        {
            SelectedListMonkeys.ForEach( x =>
            {
                var monkey = x as Monkey;
                Debug.Print(monkey.Name);
            });
        }
